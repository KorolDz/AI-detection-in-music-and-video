...
- выявлять признаки монтажа, вставок, склеек и другой цифровой обработки.
...
- forensic-модуль поиска монтажных склеек и временных разрывов в аудио, видеоряде и аудиодорожке видео без использования ИИ;
...
Для расширенного извлечения метаданных используются `ffmpeg`, `ffprobe` и `exiftool`.

Для forensic-анализа склеек и монтажа используются `ffmpeg`, `numpy`, `pydub` и `PySceneDetect`. Этот модуль не относится к проверке на ИИ и предназначен именно для поиска следов редактирования, вклеек и подозрительных переходов. Основной рабочий стек модуля сейчас консервативный и ориентирован на снижение ложных срабатываний; `librosa` оставлена в зависимостях проекта как дополнительная библиотека для дальнейшего расширения аудиоанализа.
