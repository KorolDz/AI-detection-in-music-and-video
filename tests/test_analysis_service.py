...
class AnalysisCoordinatorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.result_repository = SQLiteResultRepository(self.db_manager)
    ...
    def _fetch_result_row(self) -> sqlite3.Row:
        with closing(sqlite3.connect(self.config.db_path)) as conn:
            ...
            row = conn.execute(
                "SELECT file_name, file_path, media_type, status FROM analysis_results ORDER BY id DESC LIMIT 1"
            ).fetchone()
        assert row is not None
        return row

    def test_successful_chain_uses_temporary_copy_and_cleans_it_up(self) -> None:
        ...
        self.assertNotIn(str(media_path), result.export_payload)
    ...
    def test_unsupported_extension_stops_before_analyzer(self) -> None:
        ...
        self.assertIn("Неподдерживаемое расширение", result.error_message or "")
        self.assertEqual(
            self._fetch_event_types(),
            ["analysis_requested", "secure_intake_failed"],
        )

    def test_error_analysis_is_logged_and_returned(self) -> None:
        media_path = self._create_mp4_file()
        analyzer = FakeAnalyzerGateway(
            AnalysisResult(
                status="error",
    ...
    def test_valid_audio_is_rejected_by_business_precheck(self) -> None:
    ...
    def test_cleanup_failure_is_logged_without_overwriting_result(self) -> None:
        ...
        prepared_path.write_bytes(b"\x00\x00\x00\x18ftypisom\x00\x00\x02\x00isomiso2mp41")
    ...
    def test_url_like_source_is_rejected_before_analysis_request(self) -> None:
        ...
        self.assertTrue(result.is_error)
        self.assertFalse(analyzer.called)
        self.assertIn("только локальные файлы", (result.error_message or "").lower())
        self.assertEqual(self._fetch_event_types(), ["analysis_failed"])
        self.assertIsNone(self._fetch_result_row()["file_path"])

    def test_unc_source_is_rejected_before_analysis_request(self) -> None:
        analyzer = FakeAnalyzerGateway(
            AnalysisResult(
                status="original",
                media_type="video",
        ...
        result = coordinator.run(r"\\server\share\sample.mp4")

        self.assertTrue(result.is_error)
        self.assertFalse(analyzer.called)
        self.assertIn("сетевые пути запрещены", (result.error_message or "").lower())
        self.assertEqual(self._fetch_event_types(), ["analysis_failed"])
